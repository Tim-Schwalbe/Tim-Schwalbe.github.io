...
global.Config = window.Config; // Expose manually for non-browser env
...
console.log("Running 100% Crypto Simulation with Empirical Regime Model...");
...
let totalMDD = 0;
let finalWealths = [];
let regimeCounts = { 0: 0, 1: 0, 2: 0 }; // 0:Normal, 1:Bear, 2:Bull
...
const configs = {
    ...
    ALLOC_BONDS: 0,
    ALLOC_CRYPTO: 1,
    CRYPTO_CAGR_START: 0.60, // Default base
    CRYPTO_CAGR_END: 0.60,
    CRYPTO_VOL_START: 0.80,
    CRYPTO_VOL_END: 0.80,
    USE_FAT_TAILS: true,
    // New Params are hardcoded in market.js, inputs ignored
    PROB_CRASH: 0.005, // Ignored
    PROB_MOONSHOT: 0.005 // Ignored
};

// Generate Data (Vectorized)
// signature: generateMarketData(numSims, years, configs)
const data = window.generateMarketData(ITERATIONS, YEARS, configs);
const allCryptoReturns = data.crypto;

for (let i = 0; i < ITERATIONS; i++) {
    // Slice returns for this simulation
    // Each sim has MONTHS data points
    ...
    for (let j = 0; j < MONTHS; j++) {
        ...
        // Regime Counting (Proxy)
        if (r < -0.30) regimeCounts[1]++; // Proxy for Bear
        else if (r > 0.25) regimeCounts[2]++; // Proxy for Bull
        else regimeCounts[0]++;
    }
    totalMDD += maxDD;
