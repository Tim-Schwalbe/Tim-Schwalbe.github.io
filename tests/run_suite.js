...
(async function () {
    ...
    await runTest('Market: BEAR_MARKET_DEPTH Configuration', async () => {
        ...
        const configsDefault = {
            numSims: 100, years: 4, USE_FAT_TAILS: true, C_CAGR_START: cagr, C_CAGR_END: cagr, BEAR_MARKET_DEPTH: -0.75
        };
        const configsDeep = { ...configsDefault, BEAR_MARKET_DEPTH: -0.95 };

        const dataDefault = await generateMarketData(100, 4, configsDefault);
        const dataDeep = await generateMarketData(100, 4, configsDeep);

        let sumDef = 0, sumDeep = 0;
        for (let s = 0; s < 100; s++) {
            for (let m = 18; m < 30; m++) {
                sumDef += dataDefault.crypto[s * 48 + m];
                sumDeep += dataDeep.crypto[s * 48 + m];
            }
        }
        const avgMonthlyRetDef = sumDef / 1200;
        const avgMonthlyRetDeep = sumDeep / 1200;

        assert(avgMonthlyRetDeep < avgMonthlyRetDef, "A -95% bear depth must have a lower return than a -75% bear depth");
    });

    await runTest('Market: START_IN_BEAR_MARKET Configuration', async () => {
        const configsDefault = { numSims: 100, years: 4, USE_FAT_TAILS: true, C_CAGR_START: 1.0, START_IN_BEAR_MARKET: false };
        const configsBear = { ...configsDefault, START_IN_BEAR_MARKET: true };

        const dataDefault = await generateMarketData(100, 4, configsDefault);
        const dataBear = await generateMarketData(100, 4, configsBear);

        let sumDef = 0, sumBear = 0;
        for (let s = 0; s < 100; s++) {
            for (let m = 0; m < 12; m++) {
                sumDef += dataDefault.crypto[s * 48 + m];
                sumBear += dataBear.crypto[s * 48 + m];
            }
        }
        const avgFirstYearDef = sumDef / 1200;
        const avgFirstYearBear = sumBear / 1200;
    ...
    await runTest('Market: FORCE_CRASH TradFi Logic', async () => {
        const crashDurationYears = 2;
        const configsCrash = {
            numSims: 100, years: 4, FORCE_CRASH: true, CRASH_DURATION: crashDurationYears, CRASH_FLOOR_STOCKS: -0.05, CRASH_FLOOR_BONDS: -0.02,
            S_CAGR_START: 1.0, S_CAGR_END: 1.0, S_VOL_START: 0, S_VOL_END: 0,
            B_CAGR_START: 1.0, B_VOL_START: 0, B_VOL_END: 0,
        ...
        for (let s = 0; s < 100; s++) {
            for (let m = 0; m < 48; m++) {
                ...
                if (m < crashDurationYears * 12) {
                    assert(sRet <= -0.0499, "During crash phase, returns must be strictly capped.");
                } else {
                    assert(sRet > -0.0499, "Post-crash, natural drift must resume.");
                }
            }
        }
    });
